...
def train(opt_path):
    ...
    network_module = importlib.import_module('models.wnet')
    network_module_val = importlib.import_module('models.wnet_bsvd')
    ...
    netG = getattr(network_module, opt['model_type_train'])(net2d_opt=net2d_opt).to(device)
    netG = nn.DataParallel(netG, device_ids=[0,1])
    # netG_val = getattr(network_module_val, opt['model_type_test'])().to(device)
    ...
    for e in range(1, 42*opt.steps):
        for i, data in enumerate(train_loader):
            ...
            noise_map = data['noise_map'].unsqueeze(1).repeat(1,f,1,1,1).to(device)
            ...
            if total_step%eval_freq==0:
                # Validation
                torch.save({'params': netG.module.state_dict()}, os.path.join(model_ckpt_dir, f'ckpt_for_eval.ckpt'))
                netG_val = getattr(network_module_val, opt['model_type_test'])(pretrain_ckpt=os.path.join(model_ckpt_dir, f'ckpt_for_eval.ckpt')).to(device)
                netG_val.eval()
                #for sigma in [10,20,30,40,50]:
                for sigma in [50]:
