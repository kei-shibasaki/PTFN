...
from models.layers import NAFBlockPT, TemporalShift, NAFBlockBBBPT, MemSkip
    ...
    def __init__(self, opt, in_channels=None, out_channels=None):
        ...
        for i, num in enumerate(self.enc_blk_nums):
            for j in range(num):
                ...
                setattr(self, f'enc_block_{i}_{j}', NAFBlockPT(chan))
        ...
        for i in range(self.middle_blk_num):
            ...
            setattr(self, f'middle_block_{i}', NAFBlockPT(chan))
        ...
        for i, num in enumerate(self.dec_blk_nums):
            ...
            for j in range(num):
                ...
                setattr(self, f'dec_block_{i}_{j}', NAFBlockPT(chan))
...
class NAFDenoisingBlockBBB(nn.Module):
    def __init__(self, opt, in_channels=None, out_channels=None):
        ...
        for i, num in enumerate(self.enc_blk_nums):
            for j in range(num):
                setattr(self, f'enc_block_{i}_{j}', NAFBlockBBBPT(chan))
            setattr(self, f'enc_down_{i}', nn.Conv2d(chan, 2*chan, 2, 2))
            chan = chan * 2

        for i in range(self.middle_blk_num):
            setattr(self, f'middle_block_{i}', NAFBlockBBBPT(chan))
        ...
        for i, num in enumerate(self.dec_blk_nums):
            ...
            chan = chan // 2
            for j in range(num):
                setattr(self, f'dec_block_{i}_{j}', NAFBlockBBBPT(chan))
...
class NAFBBB(nn.Module):
    ...
    def count_shift(self):
        ...
        for name, module in self.named_modules():
            if 'NAFBlockBBBPT' in str(type(module)):
                count += 1
        return count
