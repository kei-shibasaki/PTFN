...
class PermuteToFrom(nn.Module):
    ...
    def forward(self, x, kv=None):
        ...
        *_, t, d = shape
        ...
        if kv is not None:
            axial_kv = kv.permute(*self.permutation).contiguous()
            axial_kv = axial_kv.reshape(-1, t, d)
        
        # attention
        if kv is not None:
            axial = self.fn(axial, axial_kv)
        else:
            axial = self.fn(axial)
...
class AxialTransformerBlock(nn.Module):
    def __init__(self, dim, num_dimensions=2, heads=8, dim_heads=None, mlp_ratio=4, dropout=0.0, dim_index=1, sum_axial_out=True):
        super().__init__()
        self.ln1 = ChanLayerNorm(dim)
        self.attn = AxialAttention(dim, num_dimensions, heads, dim_heads, dim_index, sum_axial_out)
        self.drop1 = nn.Dropout(dropout)
        
        self.ln2 = ChanLayerNorm(dim)
        self.mlp1 = nn.Conv2d(dim, dim*mlp_ratio, kernel_size=1)
        self.gelu = nn.GELU()
        self.drop2 = nn.Dropout(dropout)
        self.mlp2 = nn.Conv2d(dim*mlp_ratio, dim, kernel_size=1)
...
class AxialTransformerDecoderBlock(nn.Module):
    def __init__(self, dim, num_dimensions=2, heads=8, dim_heads=None, mlp_ratio=4, dropout=0.0, dim_index=1, sum_axial_out=True):
        super().__init__()
        self.ln1 = ChanLayerNorm(dim)
        self.attn1 = AxialAttention(dim, num_dimensions, heads, dim_heads, dim_index, sum_axial_out)
        self.drop1 = nn.Dropout(dropout)
        
        self.ln2 = ChanLayerNorm(dim)
        self.attn2 = AxialAttention(dim, num_dimensions, heads, dim_heads, dim_index, sum_axial_out)
        self.drop2 = nn.Dropout(dropout)
        
        self.ln3 = ChanLayerNorm(dim)
        self.mlp1 = nn.Conv2d(dim, dim*mlp_ratio, kernel_size=1)
        self.gelu = nn.GELU()
        self.drop3 = nn.Dropout(dropout)
        self.mlp2 = nn.Conv2d(dim*mlp_ratio, dim, kernel_size=1)
