    ...
    def __init__(self, opt, in_channels=None, out_channels=None):
        ...
        for i, num in enumerate(self.enc_blk_nums):
            for j in range(num):
                ...
                setattr(self, f'enc_block_{i}_{j}', DenoisingLayers(chan))
        ...
        for i in range(self.middle_blk_num):
            ...
            setattr(self, f'middle_block_{i}', DenoisingLayers(chan))
        ...
        for i, num in enumerate(self.dec_blk_nums):
            ...
            for j in range(num):
                ...
                setattr(self, f'dec_block_{i}_{j}', DenoisingLayers(chan))
...
class DenoisingBlockBBB(nn.Module):
    def __init__(self, opt, in_channels=None, out_channels=None):
        ...
        for i, num in enumerate(self.enc_blk_nums):
            for j in range(num):
                setattr(self, f'enc_block_{i}_{j}', DenoisingLayersBBB(chan))
            setattr(self, f'enc_down_{i}', nn.Conv2d(chan, 2*chan, 2, 2))
            chan = chan * 2

        for i in range(self.middle_blk_num):
            setattr(self, f'middle_block_{i}', DenoisingLayersBBB(chan))
        ...
        for i, num in enumerate(self.dec_blk_nums):
            ...
            chan = chan // 2
            for j in range(num):
                setattr(self, f'dec_block_{i}_{j}', DenoisingLayersBBB(chan))
...
class PseudoTemporalFusionNetwork(nn.Module):
    def __init__(self, opt):
        ...
        self.temp2 = DenoisingBlock(opt, in_channels=opt.width, out_channels=opt.width)
        self.temp3 = DenoisingBlock(opt, in_channels=opt.width)
        self.to_rgb1 = nn.Conv2d(in_channels=opt.width, out_channels=opt.color_channels, kernel_size=1)
        self.to_rgb2 = nn.Conv2d(in_channels=opt.width, out_channels=opt.color_channels, kernel_size=1)
    
    def forward(self, seq, noise_map):
        # seq: (B, F, C, H, W)
        b,f,c,h,w = seq.shape
        seq = self.temp1(seq, noise_map)
        inter_img1 = self.to_rgb1(seq.reshape(b*f,-1,h,w)).reshape(b,f,-1,h,w)
        seq = self.temp2(seq, noise_map)
        inter_img2 = self.to_rgb1(seq.reshape(b*f,-1,h,w)).reshape(b,f,-1,h,w)
        seq = self.temp3(seq, noise_map)
        return seq, inter_img1, inter_img2

class PseudoTemporalFusionNetworkHalf(nn.Module):
    def __init__(self, opt):
        super().__init__()
        self.temp1 = DenoisingBlock(opt, out_channels=opt.width)
...
class PseudoTemporalFusionNetworkEval(nn.Module):
    def __init__(self, opt, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
        ...
        self.temp2 = DenoisingBlockBBB(opt, in_channels=opt.width, out_channels=opt.width)
        self.temp3 = DenoisingBlockBBB(opt, in_channels=opt.width)
        self.to_rgb1 = nn.Conv2d(in_channels=opt.width, out_channels=opt.color_channels, kernel_size=1)
        self.to_rgb2 = nn.Conv2d(in_channels=opt.width, out_channels=opt.color_channels, kernel_size=1)
    ...
    def feed_in_one_element(self, seq, noise_map):
        ...
        if seq is not None:
            b,f,c,h,w = seq.shape
            inter_img = self.to_rgb1(seq.reshape(b*f,-1,h,w)).reshape(b,f,-1,h,w)
        else:
            inter_img = None
        seq = self.temp2(seq, noise_map)
        if seq is not None:
            b,f,c,h,w = seq.shape
            inter_img = self.to_rgb2(seq.reshape(b*f,-1,h,w)).reshape(b,f,-1,h,w)
        else:
            inter_img = None
        seq = self.temp3(seq, noise_map)
    ...
    def count_shift(self):
        ...
        for name, module in self.named_modules():
            if 'DenoisingLayersBBB' in str(type(module)):
                count += 1
        return count
...
# Large Model
class DenoisingBlockL(nn.Module):
    def __init__(self, opt, in_channels=None, out_channels=None):
        super().__init__()
        self.enc_blk_nums = opt.enc_blk_nums
        self.middle_blk_num = opt.middle_blk_num
        ...
        for i, num in enumerate(self.enc_blk_nums):
            for j in range(num):
                setattr(self, f'enc_tsm_{i}_{j}', TemporalShift(opt.n_frames, 'TSM', fold_div=opt.tsm_fold, stride=1))
                setattr(self, f'enc_block_{i}_{j}', DenoisingLayersL(chan))
        ...
        for i in range(self.middle_blk_num):
            setattr(self, f'middle_tsm_{i}', TemporalShift(opt.n_frames, 'TSM', fold_div=opt.tsm_fold, stride=1))
            setattr(self, f'middle_block_{i}', DenoisingLayersL(chan))
        ...
        for i, num in enumerate(self.dec_blk_nums):
            ...
            for j in range(num):
                setattr(self, f'dec_tsm_{i}_{j}', TemporalShift(opt.n_frames, 'TSM', fold_div=opt.tsm_fold, stride=1))
                setattr(self, f'dec_block_{i}_{j}', DenoisingLayersL(chan))
...
class DenoisingBlockBBBL(nn.Module):
    def __init__(self, opt, in_channels=None, out_channels=None):
        super().__init__()
        self.enc_blk_nums = opt.enc_blk_nums
        self.middle_blk_num = opt.middle_blk_num
        ...
        for i, num in enumerate(self.enc_blk_nums):
            for j in range(num):
                setattr(self, f'enc_block_{i}_{j}', DenoisingLayersBBBL(chan))
            setattr(self, f'enc_down_{i}', nn.Conv2d(chan, 2*chan, 2, 2))
            chan = chan * 2

        for i in range(self.middle_blk_num):
            setattr(self, f'middle_block_{i}', DenoisingLayersBBBL(chan))
        ...
        for i, num in enumerate(self.dec_blk_nums):
            ...
            chan = chan // 2
            for j in range(num):
                setattr(self, f'dec_block_{i}_{j}', DenoisingLayersBBBL(chan))
...
class PseudoTemporalFusionNetworkL(nn.Module):
    def __init__(self, opt):
        super().__init__()
        self.temp1 = DenoisingBlockL(opt, out_channels=opt.width)
        self.temp2 = DenoisingBlockL(opt, in_channels=opt.width)
        self.to_rgb = nn.Conv2d(in_channels=opt.width, out_channels=opt.color_channels, kernel_size=1)
...
class PseudoTemporalFusionNetworkHalfL(nn.Module):
    def __init__(self, opt):
        super().__init__()
        self.temp1 = DenoisingBlockL(opt, out_channels=opt.width)
        self.to_rgb = nn.Conv2d(in_channels=opt.width, out_channels=opt.color_channels, kernel_size=1)
...
class PseudoTemporalFusionNetworkEvalL(nn.Module):
    def __init__(self, opt, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
        super().__init__()
        self.device = device
        self.temp1 = DenoisingBlockBBBL(opt, out_channels=opt.width)
        self.temp2 = DenoisingBlockBBBL(opt, in_channels=opt.width)
        self.to_rgb = nn.Conv2d(in_channels=opt.width, out_channels=opt.color_channels, kernel_size=1)
        self.shift_num = self.count_shift()
    ...
    def count_shift(self):
        count = 0
        for name, module in self.named_modules():
            if 'DenoisingLayersBBBL' in str(type(module)):
                count += 1
        return count
...
class PseudoTemporalFusionNetworkEvalHalfL(nn.Module):
    def __init__(self, opt, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
        super().__init__()
        self.device = device
        self.temp1 = DenoisingBlockBBBL(opt, out_channels=opt.width)
