...
class DenoisingBlockBBB(nn.Module):
    ...
    def forward(self, seq):
        ...
        if seq is not None:
            ...
            x = seq.reshape(self.b*self.f,self.c,self.h,self.w)
            x = self.intro(x)
        else:
            x = None
...
class PseudoTemporalFusionNetworkEval(nn.Module):
    ...
    def feed_in_one_element(self, seq):
        #seq: (B,F,C,H,W)
        seq = self.temp1(seq)
        if seq is not None:
            b,f,c,h,w = seq.shape
    ...
    def forward(self, seq):
        ...
        with torch.no_grad():
            ...
            for i, x in enumerate(seq):
                ...
                x, inter_img = self.feed_in_one_element(x)
                out_seq1.append(x)
                out_seq2.append(inter_img)
            end_out, inter_img = self.feed_in_one_element(None)
    ...
    def forward(self, seq):
        ...
        with torch.no_grad():
            for i, x in enumerate(seq):
                # (1,C,H,W) -> (1,1,C,H,W)
                x = x.unsqueeze(0).to(self.device)
                x = self.feed_in_one_element(x)
                out_seq1.append(x)
            end_out = self.feed_in_one_element(None)
            out_seq1.append(end_out)
